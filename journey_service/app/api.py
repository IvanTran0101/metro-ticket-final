...
def gate_check_in(req: GateRequest, db: Session = Depends(get_db)):
    ...
    db.execute(
        text("UPDATE journeys SET status= 'IN_TRANSIT', check_in_time=NOW() WHERE journeys_id = :id "),
...
def gate_check_out(req: GateRequest, db: Session = Depends(get_db)):
    ...
    sch_client = SchedulerClient()
    try:
        real_fare = sch_client.calculate_fare(ticket["check_in_station_id"], req.station_id)
        real_price = flaot(real_fare["total_amount"])
...
def _finalize_journey(db, ticket, end_station):
    db.execute(
        text("UDPATE journeys SET status = 'COMPLETED', check_out_station_id= :s, check_out_time=NOW() WHERE journey_id= :id"), {"s": end_station, "id": ticket["journey_id"]}
...
def pay_penalty(req: PenaltyRequest, db: Session = Depends(get_db)):
    sql = text("SELECT * FROM journeys WHERE journey_code = :c")
    ticket = db.execute(sql, {"c": req.journey_code}).mappings().first()
